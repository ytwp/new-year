<template>
	<view>
		...
		<image
			...
			src='https://img.yeting.wang/new_year/hhu1.png'></image>
		...
		<u-divider text="压岁钱发送记录" textColor="#FFFFFF" lineColor="#ffffe0"></u-divider>
				...
				<view v-for="(item, index) in redPacket_list" :key="index"
					...
					<view style="width: 30%;">
						...
						<button v-show="item.status==10"
							...
							:data-nickname="item.nickName" :data-receivingmethod="item.receivingMethod"
		...
		<u-modal :show="login_show" content="获取头像昵称, 用于压岁钱封面展示 ~" closeOnClickOverlay showCancelButton>
		...
		<u-modal title="支付失败" :show="pay_fail_show" :showConfirmButton="false" :closeOnClickOverlay="true"
			...
			<view style="margin-top: 20rpx; width: 100%;">
				<u--text type="primary" text="请稍后在压岁钱发送记录查看" align="center" size="16"></u--text>
...
<script>
	export default {
			...
			if (res.from === 'button') {
				...
				if (dataset.nickname == null) {
					...
					if (this.currentRedPacket.receivingMethod == '打开即可') {
						receivingMethod = '压岁钱打开即可领取'
					} else if (this.currentRedPacket.receivingMethod == '写祝福语') {
						receivingMethod = '压岁钱快来领取'
					} else if (this.currentRedPacket.receivingMethod == '手势拜年') {
						receivingMethod = '压岁钱快来领取'
					}
					return {
						title: this.currentRedPacket.nickName + " 发来一份 " + receivingMethod,
					...
					return {
						title: dataset.nickname + " 发来一份 " + receivingMethod,
		...
		onLoad() {
			...
			uni.$u.http.post('/redPacket/defaultValue', {
				redPacketId: this.currentRedPacketId
			}).then(data => {
				console.log('查询支付结果：' + JSON.stringify(data))
				if (data != null) {
					data.nickName = this.from_data.nickName
					this.from_data = data
				}
			})
		},
