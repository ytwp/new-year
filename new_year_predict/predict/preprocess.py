...
def decode_base64_image(base64_data, im_info):
    imgData = base64.b64decode(base64_data)
    nparr = np.fromstring(imgData, np.uint8)
    im = cv2.imdecode(nparr, cv2.IMREAD_COLOR)  # BGR mode, but need RGB mode
    im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
    im_info['im_shape'] = np.array(im.shape[:2], dtype=np.float32)
    im_info['scale_factor'] = np.array([1., 1.], dtype=np.float32)
    return im, im_info
...
def decode_array_buffer_image(img_array, width, height, im_info):
    # 图像像素点数据，一维数组，每四项表示一个像素点的 rgba
    img = Image.new("RGBA", (width, height))  # 创建图片
    index = 0
    for j in range(0, height):
        for i in range(0, width):
            x = img_array[index]
            img.putpixel((i, j), (x[0], x[1], x[2], x[3]))
            index = index + 1
    im = cv2.cvtColor(np.asarray(img), cv2.COLOR_RGBA2RGB)
    im_info['im_shape'] = np.array(im.shape[:2], dtype=np.float32)
    im_info['scale_factor'] = np.array([1., 1.], dtype=np.float32)
    return im, im_info
